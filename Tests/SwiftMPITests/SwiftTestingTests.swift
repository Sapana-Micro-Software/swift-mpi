...
// SwiftTesting test suite for SwiftMPI framework
import Testing
@testable import SwiftMPI

/// SwiftTesting test suite for SwiftMPI framework functionality
struct SwiftMPISwiftTestingTests {
    ...
    @Test func testCommunicatorSizeAndRank() throws {
        ...
        #expect(size > 0) // Verify size is positive
        #expect(rank >= 0) // Verify rank is non-negative
        #expect(rank < size) // Verify rank is less than size
    }
    
    /// Test MPI wall clock time functionality
    @Test func testWtime() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let time1 = SwiftMPI.wtime() // Get first time measurement
        usleep(1000) // Sleep for 1 millisecond
        let time2 = SwiftMPI.wtime() // Get second time measurement
        #expect(time2 >= time1) // Verify time increases
    }
    
    /// Test MPI timer resolution
    @Test func testWtick() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let tick = SwiftMPI.wtick() // Get timer resolution
        #expect(tick > 0.0) // Verify tick is positive
    }
    
    /// Test communicator duplication functionality
    @Test func testCommunicatorDuplicate() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        ...
        let size1 = try comm1.size() // Get size from original
        let size2 = try comm2.size() // Get size from duplicate
        #expect(size1 == size2) // Verify sizes match
    }
    
    /// Test barrier synchronization operation
    @Test func testBarrier() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        #expect(throws: Never.self) { // Expect no throw
    ...
    @Test func testBroadcast() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        ...
        #expect(data[0] == 42) // Verify all processes received value
    }
    ...
    @Test func testReduce() throws {
        ...
        if rank == root { // If this is root process
            ...
            #expect(recvData[0] == expectedSum) // Verify sum is correct
        }
    }
    
    /// Test allreduce operation with sum operation
    @Test func testAllReduce() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
        ...
        let size = comm.size() // Get communicator size
        let expectedSum = Int32(size * (size + 1) / 2) // Calculate expected sum
        #expect(recvData[0] == expectedSum) // Verify all processes got sum
    }
    
    /// Test gather operation collecting data from all processes
    @Test func testGather() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
        ...
        if rank == root { // If this is root process
            for i in 0..<size { // Check each received value
                #expect(recvData[i] == Int32(i)) // Verify correct rank received
            }
        }
    }
    
    /// Test scatter operation distributing data to all processes
    @Test func testScatter() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
        ...
        #expect(recvData[0] == Int32(rank)) // Verify each process got its rank
    }
    
    /// Test allgather operation gathering data to all processes
    @Test func testAllGather() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
    ...
    @Test func testSendReceive() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
        ...
        if rank == 0 { // Process 0 sends data
            ...
            #expect(received == [42, 43, 44]) // Verify received data
        }
    }
    
    /// Test non-blocking send and receive operations
    @Test func testNonBlockingSendReceive() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
        ...
        if rank == 0 { // Process 0 sends data
            ...
            #expect(buffer == [100, 200, 300]) // Verify received data
        }
    }
    
    /// Test waitAll operation for multiple requests
    @Test func testWaitAll() throws {
        try SwiftMPI.initialize() // Initialize MPI environment
        defer { try? SwiftMPI.finalize() } // Ensure finalization on exit
        let comm = Communicator.world // Get world communicator
        let rank = comm.rank() // Get current rank
        ...
        if rank == 0 { // Process 0 sends multiple messages
            ...
            #expect(buf1[0] == 10) // Verify first message
            #expect(buf2[0] == 20) // Verify second message
        }
    }
    
    /// Test datatype definitions are accessible
    @Test func testDatatypes() {
        #expect(Datatype.int != nil) // Verify integer datatype exists
        #expect(Datatype.double != nil) // Verify double datatype exists
        #expect(Datatype.float != nil) // Verify float datatype exists
        #expect(Datatype.char != nil) // Verify character datatype exists
    }
    
    /// Test operation definitions are accessible
    @Test func testOperations() {
        #expect(Operation.sum != nil) // Verify sum operation exists
        #expect(Operation.max != nil) // Verify max operation exists
        #expect(Operation.min != nil) // Verify min operation exists
        #expect(Operation.product != nil) // Verify product operation exists
    }
}
